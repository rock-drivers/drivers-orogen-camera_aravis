...
namespace camera_aravis {
    ...
    protected:
        /* Handler for the listCameras operation
         */
        virtual ::std::string listCameras();
        ArvCamera *camera; 
        ArvStream *stream;
    public:
        base::Time lastFrameTime;
	ArvBuffer *last_buffer;
        base::samples::frame::Frame frame[50];
        base::samples::frame::Frame *frame_ptr[50];
        RTT::extras::ReadOnlyPointer<base::samples::frame::Frame> camera_frame[50];
        bool newFrame; 
        double exposure,gain;
        int new_frame_id;
//        std::vector<uint8_t> local_buffer[50];
